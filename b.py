...
admins = ["u2c7f708769a2eb35d9ae9f73cd366e0by byb"]
...
wait = {
    ...
    'timeline':False,
    'autoAdd':False,
    'message':"Owner. Nhu : ID:getk3333",
    "lang":"JP",
    "comment":"Owner. Nhu : ID:getk3333",
    "commentOn":False,
    "commentBlack":{},
    "wblack":False,
    "dblack":False,
...
#--------------------------------------------------------
              ...
              if msg.from_ in admin:
                ...
                if wait["commentOn"] == True: md+="􀜁􀇔���� Auto komentar:on 􀜁􀄯􏿿\n"
...
#-----------------------------------------------
                ...
                if msg.toType == 2:
                    ginfo = cl.getGroup(msg.to)
                    try:
                        cl.sendText(msg.to,"👉ล่ากอน"  +  str(ginfo.name)  + "")
...
☆ Ķ͈̤̱͎̱̤̞̭͂̐͒́̀͗͞Ị̵̻̝̘͍͛̏̃͊̉͠ T̩͖͎̹̫͈̿̆̏́̑́S̤̲̯̤̹̲̲̘̏̋̈́̿͒ͅŲ̶̼̲̺̣̬̔̿͐̾̾͘Ṇ̶̨̛̲̭̝̲̝̪̎̾̈́͘͢͜͞É͎̱̺̜̐̀̿͘̕̕͢  B̴̡̛͈̖̺͖̙̝̩̞̐̂̀͂̏̚͟͠O̡̩̣̲̣̜̊̑̾̾͊̃͘ͅ��̸T Ç̵͔̟̫̰̮̺̟̥̂̋̂͋͐͛͑̔̚̚O̷̧̺̠̰̳̿́͆̕̕͠ͅ N̶͖̜̻̰͍̮̼̒́̐̑͒́̕ͅŢ̢̯̱͕̠͙̤̙̄̂͗̊̈́̕R̶̛̙̩̱̗̯͌̈͆̆Ơ̴̡͈̖̺͖̙̝̩̞̐̂̀͂̏̚͟͠L̸̡̩̣̲̣̜̊̑̾̾͊̃͘͜ͅ  ☆
...
#-----------------------------------------------
                ...
                txt = msg.text.split(" ")
                jmlh = int(txt[2])
                text = msg.text.replace("run "+str(txt[1])+" "+str(jmlh)+" ","")
                tulisan = jmlh * (text+"\n")
                if txt[1] == "on":
                    if jmlh <= 1000:
                        for x in range(jmlh):
